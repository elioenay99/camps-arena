...
#### Scenario: Torneio liga é rejeitado
- **WHEN** o `tournamentId` referencia um torneio do usuário com `formato = 'liga'`
- **THEN** a action retorna erro claro explicando que partidas de liga vêm da tabela gerada, e não insere
...
AVULSO não encerrado (demais casos — incluindo torneio liga — respondem 404
único). Os selects de participante SHALL listar somente os participantes do
torneio (além de "Definir depois") e a página do torneio SHALL oferecer ao
dono o atalho "Nova partida" apenas em torneio avulso. A rota
`/dashboard/partidas/nova` SHALL atuar como seletor: lista os torneios
...
#### Scenario: Liga não tem form de partida manual
- **WHEN** o dono de uma liga abre a rota do form dela
...
#### Scenario: Seletor não lista ligas
- **WHEN** o usuário possui apenas torneios de formato liga
