...
- [x] 1.1 `supabase/schema.sql`: enum ganha `'mata_mata'` (nota: ALTER TYPE em
...
- [x] 1.4 `docs/pendencias-manuais.md`: nova seção 10 — bloco A (ALTER TYPE)
...
- [x] 2.1 `src/features/knockout/gerarChaveMataMata.ts`: tamanho da chave
...
- [x] 2.2 `src/features/knockout/gerarChaveMataMata.test.ts`: N potência e
...
- [x] 3.1 `tournamentSchema.ts`: `formato` ganha `"mata_mata"`,
      `terceiroLugar` boolean default false; schema novo do iniciar mata-mata
      (modo + cabeças/confrontos)
- [x] 3.2 `tournaments.ts` — `createTournament`: mata-mata nasce `rascunho`,
...
- [x] 3.3 `TournamentForm.tsx`: radio Mata-mata + checkboxes ida-e-volta
      (liga e mata-mata) e 3º lugar (só mata-mata), textos pt-BR

## 4. Actions de início e avanço

- [x] 4.1 `tournaments.ts` — `iniciarMataMata(prev, formData)`: sessão;
...
- [x] 4.2 `tournaments.ts` — `avancarFase(tournamentId)`: dono + mata_mata +
...
- [x] 4.3 Testes das duas actions: caminhos felizes por modo, validações,
      idempotência, 23505, sem sessão, erro único de propriedade

## 5. Lifecycle (encerrar/reabrir) com regras de mata-mata

- [x] 5.1 `match.ts` — `encerrarPartida`: em mata-mata, jogo único rejeita
...
- [x] 5.2 Testes: empate bloqueado, perna 1 empatada ok, volta antes da ida,
      agregado empatado, reabrir pós-avanço, reabrir bye, createMatch
      mata-mata rejeitado

## 6. UI — página do torneio e painel de início

- [x] 6.1 `getTournamentClassificacao`: select ganha `posicao`/`perna` e
      `terceiro_lugar`; tipos atualizados
- [x] 6.2 `src/features/knockout/components/BracketView.tsx` (RSC puro):
...
- [x] 6.3 Painel de início do mata-mata (modos sorteio/potes/manual com
      prévia via `previaMataMata`; forms com inputs nativos) + botão
      "Avançar fase" para o dono (fase completa)
- [x] 6.4 `page.tsx` do torneio: mata-mata renderiza BracketView no lugar de
      StandingsTable/clubes; mantém partidas em aberto + histórico (byes
      rotulados); painel de início por formato
- [x] 6.5 `database.types.ts`: `TournamentFormat` ganha `"mata_mata"`,
      `tournaments.terceiro_lugar`, `matches.posicao`/`perna`
- [x] 6.6 Boundaries: conferir loading/error das rotas tocadas (lição
      recorrente — toda rota nova/alterada precisa dos próprios boundaries)

## 7. Validação e fechamento

- [x] 7.1 Gates: `pnpm typecheck && pnpm lint && pnpm test && pnpm build`
- [x] 7.2 Workflow adversarial multi-agente (4 lentes + juiz); aplicados:
      must_fix (mata-mata ativo CONGELA participants — action + policy + UI,
      D11), should_fix (pré-checagem acionável no avancarFase; agregado
      revalidado ao re-encerrar a ida; testes do IniciarMataMataPanel) e nits
      (decidirConfronto defensivo, role=alert, placeholder do 3º lugar,
      comentários de limite conhecido); re-verificado por agente focado e
      gates re-rodados (463/463)
