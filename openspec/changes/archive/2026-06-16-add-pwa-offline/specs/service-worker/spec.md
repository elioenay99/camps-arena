...
Para esses, SHALL aplicar `stale-while-revalidate`. Todo o restante
SHALL fazer bypass (network-only, sem gravar), explicitamente incluindo: requisições
cross-origin (Supabase, Storage, realtime, Sentry, analytics), requisições não-`GET`
(Server Actions/mutações), payloads RSC (`_rsc` na query ou header `RSC`),
`/_next/image` (imagens otimizadas), rotas de dados/imagem same-origin (p.ex.
