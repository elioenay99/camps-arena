...
- [x] 1.1 SQL mostrado ao dono ANTES (REGRA 4), bloco único comentado.
- [x] 1.2 Tabelas `tournament_members`, `league_members` (papel CHECK
...
- [x] 1.3 `liga_do_torneio(uuid)` DEFINER STABLE `search_path=''` — UNION das 5 colunas
  (apertura/clausura/final + `league_boundaries.playoff_tournament_id`). Grant anon+auth.
- [x] 1.4 Helpers torneio `pode_gerir/arbitrar/moderar_torneio` + **`pode_ver_bastidores_torneio`**
...
- [x] 1.5 **Refatorar `lock_match_lifecycle`**: troca `t.created_by=auth.uid()` por
  `public.pode_arbitrar_torneio(new.tournament_id)` (mantém defesa de coluna + bypass
  service_role). **[must_fix #1]**
- [x] 1.6 Trigger `lock_tournament_reopen` BEFORE UPDATE OF status ON tournaments (DEFINER,
...
- [x] 1.7 RPCs: `info_convite_membro(text)` DEFINER (preview `{escopo,id,titulo,papel,
...
- [x] 1.8 Policies das 3 tabelas novas: `*_members` SELECT `pode_gerir_*` OR próprio;
...
- [x] 1.9 Refactor das policies dono→capacidade (tabela §6):
...
- [x] 1.10 league_* policies (enumerar uma a uma, distinguindo direta de transitiva):
...
- [x] 1.11 Aplicar via MCP em PROD (`bfxmdypdxbbfedtqsqik`); espelhar `schema.sql`,
...
- [x] 2.1 `src/lib/autorizacao.ts`: `podeGerir/podeArbitrar/podeModerar(supabase,
  {tournamentId}|{competitionId})` via `.rpc()`. Mensagem de negação única.
- [x] 2.2 `tournaments.ts`: iniciar/avançar/encerrar/cores→gerir; liberar rodadas→arbitrar;
  **reabrir torneio + crash-recovery ativo→rascunho permanecem dono-only** (`eq created_by`).
- [x] 2.3 **Varredura ESCOPADA** (NÃO cega): trocar `created_by`→**podeArbitrar** SÓ em
...
- [x] 2.4 `leaguePyramid.ts`: iniciar divisão / montar playoffs / grandes finais / calcular
...
- [x] 2.5 Loaders RSC de liga (`getSeason`/`getCompetition` e afins) **deixam de filtrar
  `created_by`** e gateiam por `pode_gerir_competition`. **[must_fix #5]**
- [x] 2.6 **moderar**: convites de participante/vaga (gerar/regenerar/remover), expulsar/
...
- [x] 3.1 `src/actions/equipe.ts`: `gerarConviteMembro` (papel ∈ **árbitro/moderador** —
...
- [x] 3.2 `buscarUsuarios(query)`: `users_public` (id/nome/avatar), min 2 chars, limit 8,
  autenticado, **exclui caller + membros/dono**. Sem PII.
- [x] 3.3 Schemas Zod `src/schema/equipe.ts` (papel, escopo, code).

## 4. UI

- [x] 4.1 `src/features/team-roles/`: `TeamSection` (lista+remover), `MemberInviteCards`
  (link **só p/ árbitro/moderador**), `AddMemberSearch` (busca+papel; **admin só aparece
  p/ o dono**). 390px, 2 temas.
- [x] 4.2 Rotas `/dashboard/torneios/[id]/equipe` + `/dashboard/ligas/[id]/equipe` (gate
  capacidade gerir). **`/cores` (torneio e liga) deixam de filtrar `created_by`** → gate
  por capacidade. **[must_fix #5]**
- [x] 4.3 Rota `/equipe/convite/[code]` + `AcceptTeamInviteForm` (usa `info_convite_membro`).
- [x] 4.4 `page.tsx` torneio + página da pirâmide: derivar `capacidades` e mapear **CADA
...
- [x] 5.1 `adicionarMembro` e `aceitar_convite_membro` (app-layer): após inserir o membro,
  enviar via `subscriptions_para_nomeacao` (best-effort). "Você virou &lt;papel&gt; em
  &lt;campeonato&gt;". **[must_fix #4]**

## 6. Testes

- [x] 6.1 Unit: autorização (gerir/arbitrar/moderar por papel); `buscarUsuarios` (min
  chars, sem PII, exclui membros); **paridade do vocabulário de papéis** entre tabelas e
  helpers.
- [x] 6.2 RPC `aceitar_convite_membro` (válido/inválido/já-dono/já-membro/idempotente;
...
- [x] 6.3 RLS reais via SQL (`auth.uid()` simulado): **árbitro encerra/reabre partida E
...
- [x] 7.1 `pnpm typecheck && pnpm lint && pnpm test && pnpm build` (PIPESTATUS).
- [x] 7.2 Workflow de REVIEW adversarial do diff → corrigir HIGH/CRITICAL.
- [x] 7.3 Validação ao vivo (chrome-devtools, 2 contas, 390px, 2 temas): dono adiciona
  ...
  pirâmide.
- [x] 7.4 Commit pt-BR (sem coautoria) + push.
- [x] 7.5 `openspec archive add-equipe-campeonato` + atualizar memória de retomada.
