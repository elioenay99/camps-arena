...
- [x] 1.1 Refatorar `src/features/og/brand.tsx`: `export` de `carregarAssets()` (fontes +
  logo) sem mudar o comportamento do brand estático.
- [x] 1.2 `src/features/og/rodada.tsx`: `renderRodadaOg({ titulo, rodada, confrontos, cores })`
...
- [x] 1.3 Helper interno: escudo `escudoUrl → dataURL` via `fetch(url,{signal:
...
- [x] 2.1 `app/dashboard/torneios/[id]/rodada/[rodada]/imagem/route.tsx` (GET): `createClient`
...
- [x] 2.2 Carrega confrontos via `getPartidasDaRodada` + cores via `resolverCoresTorneio`
  (passando o `torneio` já carregado); chama `renderRodadaOg`; devolve o `ImageResponse`.
- [x] 2.3 **Adicionar a rota ao `outputFileTracingIncludes` do `next.config.ts`**
...
## 3. Dados + texto
- [x] 3.1 `src/features/match/data/getPartidasDaRodada.ts`: select `matches` por
...
- [x] 3.2 `mensagemRodada(...)` em `src/lib/whatsapp.ts` (fonte única, sem emoji decorativo,
...
- [x] 4.1 `CompartilharRodadaButton` (client, `src/features/match/components/`): Web Share
  ...
  titulo, texto}`.
- [x] 4.2 `app/dashboard/torneios/[id]/page.tsx`: na seção "Liberação de rodadas", abaixo de
...
## 5. Testes
- [x] 5.1 `getPartidasDaRodada`: select certo; nome (`team?.nome ?? rotulo`); `porNome`;
  `escudoUrl`; **dedup ida-e-volta** (perna 1+2 do mesmo par → 1 confronto); **bye/lado vazio
  pulado**.
- [x] 5.2 `mensagemRodada`: linha por confronto, comandante **+ wa.me** vs ❌, header, URL
  absoluta, sem emoji decorativo, fallbacks; encerrada sai sem wa.me.
- [x] 5.3 `CompartilharRodadaButton`: share quando `canShare({files})`; fallback quando não
  (clipboard + open wa.me); `AbortError` não vira erro de toast.
- [x] 5.4 Rota de imagem: nega não-dono (404). Lógica pura de confrontos/monograma testada à
  parte (o ImageResponse valida-se ao vivo).

## 6. Gates + review + validação
- [x] 6.1 `pnpm typecheck && pnpm lint && pnpm test && pnpm build` (`${PIPESTATUS[0]}`).
- [x] 6.2 Workflow de review adversarial do diff → corrigir HIGH/CRITICAL.
- [x] 6.3 Validação ao vivo (LOCAL, 390px, conta de teste): liberar rodada → botão aparece
...
## 7. Encerramento
- [x] 7.1 Commit pt-BR (sem coautoria) + push. (Sem DDL → sem MCP.)
- [x] 7.2 `openspec archive add-compartilhar-rodada`; atualizar memória de retomada.
