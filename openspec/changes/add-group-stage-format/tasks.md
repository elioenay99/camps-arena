...
- [x] 1.1 `supabase/schema.sql`: enum ganha `'grupos_mata_mata'` e
...
- [x] 1.2 `supabase/schema.sql`: `lock_match_relations` trava `grupo`;
...
- [x] 1.3 `docs/pendencias-manuais.md`: seção 12 (Run A: 2 ALTER TYPE;
...
- [x] 2.1 `src/features/groups/gerarFaseDeGrupos.ts`: `montarGrupos`
...
- [x] 2.2 Testes do motor: equilíbrio p/ N não-múltiplo; potes 1 cabeça/grupo;
...
- [x] 3.1 `gerarChaveMataMata.ts`: `tamanhoChaveDasPartidas`/
...
- [x] 4.3 `tournaments.ts` — `gerarMataMataDosGrupos(tournamentId)`: grupos
...
- [x] 4.4 `avancarFase`: aceita os 3 formatos com chave; opera nas partidas
      com posicao (rodada-base); rejeita grupos sem chave c/ orientação
- [x] 4.5 `participants.ts` — `chaveEmAndamento`: cobre os 3 formatos
- [x] 4.6 Testes das actions (frentes paralelas)

## 5. UI

- [x] 5.1 `TournamentForm`: radios Grupos+mata-mata e Fase de liga (com
      ida-e-volta/3º lugar)
- [x] 5.2 `getTournamentClassificacao`: select + projeções `grupos`
      (tabelas por grupo via computeStandings por subconjunto) e rótulo de
      grupo nas listas; tipos
- [x] 5.3 Painel de início de grupos (G/K/modo, prévia, validação UX) +
      botão "Gerar mata-mata" (orientativo enquanto faltam jogos; toast de
      sorteio de corte)
- [x] 5.4 `page.tsx`: render por formato (tabelas por grupo + chave);
      `listaCongelada` cobre os formatos novos; subtítulo
- [x] 5.5 `database.types.ts`: enum + colunas novas
- [x] 5.6 Testes de componentes e fetcher (frentes paralelas)

## 6. Validação e fechamento

- [x] 6.1 Gates: `pnpm typecheck && pnpm lint && pnpm test && pnpm build`
- [x] 6.2 Workflow adversarial (4 lentes + juiz): changes_required → 2
      must_fix aplicados (promote-first no iniciar de grupos — corrida de
      partições divergentes escapava do índice, provado numericamente; painel
      com defaults derivados/valores efetivos + G>=2 em grupos) + 4
      should_fix (lifecycle por FORMATOS_COM_CHAVE com regras de grupo;
      promessa do cruzamento corrigida; pino G=2 K=4; retry de K resolvido
      pelo promote-first) + nits; re-verificado por agente focado
      (verificação numérica exaustiva N=2..32); gates re-rodados (606/606)
- [x] 6.3 Commits (proposal/impl/archive) + push + CI verde
- [x] 6.4 Archive + memória + lembrar o usuário da seção 12
