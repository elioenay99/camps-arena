...
`--frozen-lockfile` e Node 22 LTS, garantindo builds reproduzíveis. O pipeline SHALL
executar a suíte sem depender de segredos de produção (sem `service_role`,
`DATABASE_URL` ou `API_FOOTBALL_KEY`), pois os testes são herméticos (usam mocks).
